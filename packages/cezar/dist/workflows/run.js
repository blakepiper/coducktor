...
import { resolveReasoningEffort } from '../core/reasoning.js';
...
import { classifyRunnerFailure } from '../core/quota/failure-classifier.js';
import { isAutoProvider } from '../core/runner-selection.js';
...
function isQuotaBlocked(value) {
    return typeof value === 'object' && value !== null && value.kind === 'quota-blocked';
}
...
export class RunManager {
    ...
    dispose() {
        ...
        for (const timer of this.quotaWakeTimers.values())
            clearTimeout(timer);
            ...
            do {
                ...
                while (this.queue.length > 0 && capacity()) {
                    ...
                    const next = this.queue.findIndex((id) => {
                        ...
                        if (queued?.blockedReason?.type === 'provider_quota')
                            return false;
                    ...
                    if (continuation) {
                        ...
                        void this.runContinuation(runId, hydrated.stepId, hydrated.sessionId, hydrated.backend, hydrated.prompt, hydrated.images, hydrated.persistedImages, hydrated.persistedAttachments, hydrated.context).catch((err) => {
    ...
    persistedImages = [], persistedAttachments = [], context) {
        ...
        if (refreshAfterTurn && refreshPrompt) {
            ...
            await this.runContinuation(runId, stepId, undefined, continueBackend, refreshPrompt, [], [], [], {
    ...
    async execute(runId, workflow, input) {
        ...
        if (taskBackend !== 'auto' && this.requeueWhileHeld(runId, workflow, input, taskBackend))
            return;
        ...
        let modelIdentity;
        try {
            const normalized = taskBackend === 'auto' ? undefined : normalizeModelForBackend(taskBackend, agentModelsLocked(this.repoRoot) ? undefined : input.model, { configuredProvider: await configuredModelProvider(taskBackend, this.repoRoot) });
        ...
        if (state.cwd === this.repoRoot) {
            ...
            if (taskBackend !== 'auto' && this.requeueWhileHeld(runId, workflow, input, taskBackend, state))
                return;
        }
        ...
        while (i < workflow.steps.length) {
            ...
            if (kind === 'agent') {
                ...
                let failure;
                do {
                    failure = await this.runAgentStep(runId, state, step, input, skills, checkFailure, interactive, emit, startImages, taskBackend, extraSystemPrompt, chainStepNote(workflow.steps, i), startAttachments, attemptedProviders, attemptedProviders.size > 0 ? QUOTA_FAILOVER_PREAMBLE : undefined);
    ...
    blockForProviderQuota(runId, workflow, input, state, retryAt) {
        const run = this.store.getRun(runId);
        ...
        if (run.currentStepId)
            this.store.updateStep(runId, run.currentStepId, { status: 'pending', error: undefined });
        this.store.updateRun(runId, {
            status: 'queued',
            currentStepId: undefined,
            error: undefined,
     ...
     */
    parkActiveForProviderQuota(runId, state) {
        state.releaseRepoRoot?.();
        state.releaseRepoRoot = undefined;
        this.clearIdleTimer(state);
        this.clearMonitoringWakeTimer(state, runId);
    ...
    wakeQuotaBlockedRuns() {
        if (![...this.queue, ...this.quotaWakeTimers.keys()].some((id) => this.store.getRun(id)?.blockedReason?.type === 'provider_quota'))
            return;
    ...
    async resolveRunnerSelection(selection, attemptedProviders) {
        if (selection !== 'auto')
            return { backend: selection };
        if (!this.quotaCoordinator)
            return 'quota-aware routing is unavailable';
        ...
        if (result.kind === 'wait')
            return { kind: 'quota-blocked', retryAt: result.retryAt };
    ...
    attachments = [], attemptedProviders, failoverPreamble, contextRefreshPromptOverride) {
        let systemPrompt;
        if (step.skill) {
        ...
        if (refreshAfterTurn && refreshPrompt) {
            ...
            return this.runAgentStep(runId, state, step, input, skills, checkFailure, interactive, emit, undefined, taskBackend, extraSystemPrompt, chainNote, [], attemptedProviders, failoverPreamble, refreshPrompt);
        }
        return result;
    }
    /**
    ...
    takeContextRefreshPrompt(state) {
        const pending = state.pendingContextRefresh;
        if (!pending || !this.semaphore.intelligentContextRefresh() || state.cancelled)
            return undefined;
