 ...
 */
async function bootstrapVerified(ctx, uid, label, path, what) {
    ...
    if (code !== 0 || !loaded) {
        throw new StepAborted(`launchctl could not load ${what} (bootstrap exit ${code}) — inspect it with: launchctl print gui/${uid}/${label}`);
    }
}
...
export function launchdPlist(port, basicAuth, domain, ngrokBin = '/opt/homebrew/bin/ngrok') {
...
const ngrokStep = {
    ...
    async run(ctx) {
        ...
        if (!present) {
            if (ctx.dryRun)
                ctx.ui.info('DRY RUN — would run: brew install ngrok/ngrok/ngrok');
            else
                await ctx.runner.interactive('brew', ['install', 'ngrok/ngrok/ngrok']);
        }
        ...
        if (!ctx.dryRun) {
            ...
            if (code !== 0)
                throw new StepAborted('ngrok rejected the authtoken (`ngrok config add-authtoken` failed) — check the token and re-run');
        }
    ...
    async undo(ctx, created) {
        ...
        if (cfg) {
            ctx.ui.note(`The ngrok authtoken was left in ngrok's own config — remove it yourself if you want it gone:\n${cfg.removeHint ?? ''}`, 'ngrok');
        }
    },
};
...
const cezarPlistPath = () => join(homedir(), 'Library', 'LaunchAgents', `${CEZAR_PLIST_LABEL}.plist`);
/** Resolve the argv array for the cezar launchd agent, mirroring how the CLI was launched. */
async function resolveCezarArgv(ctx) {
    const node = process.execPath;
    const pkgRoot = resolve(fileURLToPath(new URL('../../..', import.meta.url)));
    const entry = join(pkgRoot, 'dist', 'index.js');
    const npxPath = join(dirname(node), 'npx');
    if (/[/\\]_npx[/\\]/.test(pkgRoot))
        return [npxPath, '--yes', OFFICIAL_CLI_PKG];
    if (ctx.dryRun || existsSync(entry))
        return [node, entry];
    ...
    throw new StepAborted(`could not locate a runnable cezar (${entry} missing, no global ${OFFICIAL_CLI_PKG}) — ` +
...
export function cezarLaunchdPlist(repoRoot, port, argv) {
...
const identityStep = {
    ...
    async run(ctx) {
        ...
        if (up)
            ctx.ui.success('ngrok tunnel is up (basic-auth enforced at the ngrok edge).');
        else
...
export const macosxNgrok = {
    ...
    async redeploy(ctx) {
        ...
        if (cezarCode !== 0)
            ctx.ui.warn(`launchctl kickstart returned non-zero — check \`launchctl print gui/${uid}/${CEZAR_PLIST_LABEL}\`.`);
        ...
        if (code !== 0)
            ctx.ui.warn(`launchctl kickstart returned non-zero — check \`launchctl print gui/${uid}/${PLIST_LABEL}\`.`);
        await identityStep.run(ctx);
    },
};
//# sourceMappingURL=macosx-ngrok.js.map