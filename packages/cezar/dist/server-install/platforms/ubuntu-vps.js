 ...
 */
export async function portListener(ctx, port) {
    if (ctx.dryRun)
        return null;
...
export function upstreamHost(ctx) {
 ...
 */
export function nginxVhost(port, serverName = '_', htpasswd = '/etc/cezar/htpasswd') {
...
const nginxProxyStep = {
    ...
    async undo(ctx, created) {
        ...
        await sudoStep(ctx, {
            ...
            command: `rm -f ${vhostEnbl} ${vhostAvailable(ctx)} ${htpasswdPath(ctx)}` +
                restoreClause +
    ...
    async run(ctx) {
        ...
        if (vhostHasTls) {
            await sudoStep(ctx, {
                ...
                verify: (c) => verifyCommand(c, 'sh', ['-c', `grep -qF ${shquote(`server_name ${trimmedDomain}`)} ${vhostAvail}`]),
            });
        }
        else {
 ...
 */
export function systemdUnit(repoRoot, port, scope, execStart, bindHost) {
 ...
 *  or a global bin. */
export function isNpxExecStart(execStart) {
 ...
 * directly).
 */
export function refreshNpxCacheForRedeploy(ctx, execStart) {
...
async function readExecStart(ctx, scope, unit) {
    ...
    async run(ctx) {
        ...
        if (userBus) {
            ...
            if (ctx.dryRun) {
                ...
                writeFileSync(unitPath, systemdUnit(ctx.repoRoot, ctx.state.primaryPort, 'user', execStart, ctx.state.bindHost), 'utf8');
 ...
 */
async function verifyBehindExternalProxy(ctx) {
    ...
    if (!(await waitForCezar(ctx, port))) {
        ctx.ui.error(`cezar is not answering on ${target}.\n\n` +
    ...
    ctx.ui.warn('cezar has NO built-in authentication in this mode — your reverse proxy MUST enforce it. ' +
...
const identityStep = {
    ...
    async run(ctx) {
        ...
        if (ctx.state.externalProxy)
            return verifyBehindExternalProxy(ctx);
        if (ctx.dryRun) {
...
export const ubuntuVps = {
    ...
    async preflight(ctx) {
        ...
        if (!ctx.state.externalProxy) {
            ...
            if (holder && !/nginx/i.test(holder)) {
                ctx.ui.warn(`Port 80 is already served by "${holder}" on this host.\n` +
    ...
    steps(ctx) {
        ...
        if (ctx.state.externalProxy)
            return [depCheckStep(), autostartStep, identityStep];
