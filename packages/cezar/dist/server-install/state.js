...
export function loadServerState(instance = DEFAULT_SERVER_INSTANCE) {
 ...
 */
export function listServerInstances() {
    const out = [];
 ...
 */
export function nextFreeInstancePort(startAt = 4321) {
...
export function saveServerState(state, instance = DEFAULT_SERVER_INSTANCE) {
    const path = serverStatePath(instance);
 ...
 */
export function deleteServerState(instance) {
    if (instance === DEFAULT_SERVER_INSTANCE)
        return;
    try {
...
export function isResolved(outcome) {
 ...
 */
export function firstIncompleteStep(orderedIds, state) {
 ...
 * release function.
 */
export function acquireLock(instance = DEFAULT_SERVER_INSTANCE) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        try {
            ...
            return () => {
                ...
                released = true;
                try {
                    if (existsSync(path) && readLockPid(path) === process.pid)
                        rmSync(path);
                }
                catch {
                    // non-fatal
            ...
            if (holder !== null && holder !== process.pid && isProcessAlive(holder)) {
                throw new LockHeldError(`another server-install/uninstall is already running (pid ${holder}). ` +
    ...
    throw new LockHeldError(`could not acquire ${path} — another server-install/uninstall grabbed it first; retry in a moment.`);
}
