 ...
 */
function recordOutcome(state, id, status) {
 ...
 */
function reconcileDryRun(state, opts, ui) {
...
export async function runInstall(strategy, opts) {
    ...
    try {
        ...
        if (state.platform && state.platform !== strategy.id) {
            throw new PreflightError(`this host already has a ${state.platform} install recorded — run \`server-uninstall\` first to switch platforms`);
        }
        ...
        if (opts.externalProxy !== undefined)
            state.externalProxy = opts.externalProxy;
        if (opts.bindHost)
            state.bindHost = opts.bindHost;
        ...
        if (unknownIds.length > 0) {
            throw new PreflightError(`unknown --reconfigure step id(s): ${unknownIds.join(', ')} — ` +
...
export async function runUninstall(strategy, opts) {
    const release = acquireLock(opts.instance);
    try {
        const state = loadServerState(opts.instance);
        const ctx = buildContext(state, opts);
        ...
        if (state.platform && state.platform !== strategy.id) {
            throw new PreflightError(`this host has a ${state.platform} install recorded — run \`server-uninstall --platform ${state.platform}\``);
        }
        const registeredProjects = (await loadWorkspaceConfig()).projects.length;
        if (registeredProjects > 0) {
            ctx.ui.warn(`${registeredProjects} ${registeredProjects === 1 ? 'project is' : 'projects are'} registered in ~/.cezar/config.json.`);
        ...
        if (leftover.length > 0) {
            ctx.ui.warn(`Not reversed (recorded by a different cezar version): ${leftover.join(', ')}. ` +
        ...
        if (ctx.instance !== 'default')
            deleteServerState(ctx.instance);
        return { status: 'complete', state };
    }
    finally {
        release();
 ...
 */
export async function runDeploy(strategy, opts) {
    const release = acquireLock(opts.instance);
    try {
        const state = loadServerState(opts.instance);
        const ctx = buildContext(state, opts);
        if (state.platform && state.platform !== strategy.id) {
            throw new PreflightError(`this host has a ${state.platform} install recorded — deploy it with --platform ${state.platform}`);
        }
        if (!state.installed) {
            ctx.ui.warn(`no completed ${strategy.label} install is recorded on this host — run \`server-install --platform ${strategy.id}\` first.`);
            return { status: 'failed', state };
        }
