...
import { isRunnerId } from '../core/agent-runner.js';
...
export async function verifyCommand(ctx, program, args, matcher = (r) => r.code === 0) {
    if (ctx.dryRun)
        return false;
 ...
 */
export async function hasPasswordlessSudo(ctx) {
    if (ctx.dryRun)
        return false;
...
export function generatePassword(length = 16) {
    ...
    const pick = (set) => set.charAt(randomInt(set.length));
...
export async function sudoStep(ctx, opts) {
    const { ui } = ctx;
    ...
    for (;;) {
        ...
        if (mode === 'sudo') {
            const code = await ctx.runner.interactive('sudo', ['bash', '-lc', opts.command], opts.input != null ? { input: opts.input } : undefined);
            if (code !== 0)
                ui.warn(`command exited with code ${code}`);
        }
        else {
...
export function depCheckStep(opts = {}) {
    ...
    return {
        ...
        async run(ctx) {
            ...
            const pick = await ctx.ui.multiselect({
            ...
            for (const name of pick) {
                ...
                if (check?.hint)
                    ctx.ui.note(check.hint, `Authorize ${name}`);
            }
...
export const aptInstallTool = async (ctx, name) => {
    if (name === 'gh') {
        await sudoStep(ctx, {
...
export const brewInstallTool = async (ctx, name) => {
    if (name === 'gh') {
        if (ctx.dryRun) {
...
async function installViaNpmOrNote(ctx, name, sudo) {
    if (name === 'opencode') {
