...
export async function discoverSkills(repoRoot) {
    ...
    const teamSkills = filterImportedTeamSkills(getTeamSkillsCached(repoRoot), gatedRepos, readImportedSkills(uiState));
    const merged = [];
    const seen = new Set();
 ...
 */
export function readImportedSkills(uiState) {
 ...
 */
export function filterImportedTeamSkills(teamSkills, gatedRepos, importedSkills) {
    // Not curated → the full default catalog still appears (no upgrade break).
    if (importedSkills === undefined)
        return [...teamSkills];
    const imported = new Set(importedSkills);
    return teamSkills.filter((skill) => !skill.team || !gatedRepos.has(skill.team.repo) || imported.has(skill.name));
}
/**
 ...
 */
async function skillEntryPaths(dir, depth, visited) {
    if (depth < 0)
        return [];
    let real;
    try {
...
async function readMarkdownSkills(dir, source) {
