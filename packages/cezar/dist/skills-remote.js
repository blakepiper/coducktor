 ...
 */
export function safeRemoteFor(repo) {
...
export function isPinnedSha(ref) {
...
const warnedUnsafeRemotes = new Set();
function warnUnsafeRemoteOnce(repo) {
    if (warnedUnsafeRemotes.has(repo))
        return;
    warnedUnsafeRemotes.add(repo);
    console.warn(`[cez] ignoring skills repo ${JSON.stringify(repo)} — not an accepted source shape ` +
...
async function resolveRef(bareDir, ref) {
    ...
    if (isPinnedSha(ref)) {
        ...
        const probe = await git(['rev-parse', '--verify', '--quiet', `${sha}^{commit}`], LIST_TIMEOUT_MS, bareDir);
    ...
    for (const candidate of [ref, `refs/heads/${ref}`, 'HEAD']) {
        const probe = await git(['rev-parse', '--verify', '--quiet', `${candidate}^{commit}`], LIST_TIMEOUT_MS, bareDir);
 ...
 */
export async function readRemoteSkill(src, path, atCommit) {
...
const teamSkillsByRoot = new Map();
const firstLoadByRoot = new Map();
function initialTeamSkillsLoad(repoRoot) {
 ...
 */
export function waitForTeamSkills(repoRoot) {
