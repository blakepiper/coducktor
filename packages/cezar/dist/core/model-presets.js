import { BACKEND_MODEL_MAP, parseModelIdentity } from './model-identity.js';
/**
 ...
 */
const KNOWN_PROVIDERS = ['anthropic', 'openai', 'google', 'openrouter'];
/**
 ...
 */
export const KNOWN_PRESETS_BY_RUNNER = {
    claude: [
        'opus',
        'sonnet',
        'haiku',
 ...
 */
function namesAnotherKnownProvider(model, runner) {
    ...
    if (map?.defaultProvider === undefined || map.allowExplicitProvider)
        return false;
    const explicit = parseModelIdentity(model);
    if (!explicit || explicit.provider === map.defaultProvider)
        return false;
    return KNOWN_PROVIDERS.includes(explicit.provider);
}
...
export function modelConflictsWithRunner(model, runner) {
    if (!model)
        return false;
    if (KNOWN_PRESETS_BY_RUNNER[runner]?.includes(model))
        return false;
    ...
    if (namesAnotherKnownProvider(model, runner))
        return true;
    return Object.entries(KNOWN_PRESETS_BY_RUNNER).some(([other, presets]) => other !== runner && presets.includes(model));
}
