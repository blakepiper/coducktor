 ...
 */
export function renderEnvPrefix(env, platform) {
    const entries = Object.entries(env);
    if (entries.length === 0)
        return '';
    for (const [name, value] of entries) {
        if (CONTROL_CHARS_RE.test(name) || CONTROL_CHARS_RE.test(value))
            return null;
        if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(name))
            return null;
        if (platform === 'win32' && (WIN32_UNSAFE_RE.test(name) || WIN32_UNSAFE_RE.test(value)))
            return null;
    }
    if (platform === 'win32') {
...
export function withEnvPrefix(command, env, platform) {
