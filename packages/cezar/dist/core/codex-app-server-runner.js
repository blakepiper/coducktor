...
class CodexSession {
    ...
    handleUserInputRequest(rpcId, params) {
    ...
    isForeignTurnLifecycle(msg) {
        const method = typeof msg.method === 'string' ? msg.method : undefined;
        if (!method || !TURN_LIFECYCLE_METHODS.has(method))
            return false;
        return this.isForeignThreadTurn((msg.params ?? {}));
    }
    handleNotification(method, params) {
        switch (method) {
            ...
            case 'item/agentMessage/delta': {
                ...
                if (delta)
                    this.textCoalescer.append(stringField(params, 'itemId'), delta);
                break;
            }
            case 'item/started': {
            ...
            case 'turn/failed': {
                if (this.isForeignThreadTurn(params))
                    break; // don't end the parent turn on a child turn (#600)
                this.pendingUserInput = undefined;
...
// ---- helpers --------------------------------------------------------------
function codexAskQuestions(value) {
    ...
    for (const raw of value) {
        ...
        if (!id || !header || !prompt || !Array.isArray(question.options))
            return null;
...
function userInputAnswers(questions, text) {
 ...
 */
export function reasoningSummary(env = process.env) {
...
//# sourceMappingURL=codex-app-server-runner.js.map