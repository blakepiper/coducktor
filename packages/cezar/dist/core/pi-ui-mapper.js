...
export function createPiUiState() {
    return {
...
export function piTurnStarted(state) {
...
export function mapPiRpcMessage(value, state) {
...
function mapMessageUpdate(value, state) {
    ...
    if (!update || !updateType || !state.turnId)
        return { events: [], state };
    if (updateType === 'done') {
        return {
            events: [],
    ...
    if (updateType === 'error') {
        const reason = string(update.reason) === 'aborted' ? 'cancelled' : 'error';
    ...
    const field = updateType.startsWith('thinking_') ? 'reasoning' : updateType.startsWith('text_') ? 'text' : null;
    if (!field)
        return { events: [], state };
    ...
    const makeItem = (text) => field === 'text'
...
function mapToolStart(value, state) {
    ...
    tools.set(id, item);
    const events = [{ type: 'item.started', item }];
    const plan = toolPlan(name, value.args);
    if (plan)
        events.push({ type: 'plan.updated', entries: plan });
    return { events, state: { ...state, tools } };
}
function mapToolUpdate(value, state) {
    const id = string(value.toolCallId);
    ...
    if (output === undefined)
        return { events: [], state };
    const item = { ...previous, output };
    const tools = new Map(state.tools);
    tools.set(id, item);
    return { events: [{ type: 'item.updated', item }], state: { ...state, tools } };
}
function mapToolEnd(value, state) {
    const id = string(value.toolCallId);
    const previous = id ? state.tools.get(id) : undefined;
    if (!id || !previous)
        return { events: [], state };
...
function completeTurn(reason, state) {
    if (!state.turnId)
        return { events: [], state };
    const event = {
        type: 'turn.completed',
    ...
    if (state.turnUsage)
        event.usage = state.turnUsage;
    if (state.turnCostUsd !== null)
        event.costUsd = state.turnCostUsd;
...
function mapMessageEnd(value, state) {
...
function toolDiffs(name, input) {
    if (!isRecord(input) || !['edit', 'write'].includes(name.toLowerCase()))
        return undefined;
...
function toolPlan(name, input) {
    ...
    for (const todo of input.todos) {
        ...
        if (!content || !['pending', 'in_progress', 'completed', 'cancelled'].includes(status ?? ''))
            continue;
        entries.push({ content, status: status });
    }
    return entries;
}
...
//# sourceMappingURL=pi-ui-mapper.js.map