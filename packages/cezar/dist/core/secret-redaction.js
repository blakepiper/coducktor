 ...
 */
export const SECRET_NAME_RE = /(TOKEN|SECRET|PASSWORD|PASSWD|CREDENTIAL|_KEY$|_KEY_|APIKEY|API_KEY|PRIVATE_KEY|ACCESS_KEY|_AUTH$|_AUTH_|SESSION|COOKIE|PASSPHRASE)/i;
...
const SECRET_VALUE_NAME_ALLOW = new Set([
    'SSH_AUTH_SOCK',
    'SSH_AGENT_PID',
    'SESSION_MANAGER',
    'SESSIONNAME',
...
export function collectSecretValues(env = process.env) {
    ...
    for (const [name, value] of Object.entries(env)) {
        if (!value || value.length < MIN_SECRET_LEN)
            continue;
        if (SECRET_VALUE_NAME_ALLOW.has(name.toUpperCase()))
            continue;
...
export function redactDeep(value, secretValues) {
    ...
    if (Array.isArray(value)) {
        return value.map((v) => redactDeep(v, secretValues));
    }
