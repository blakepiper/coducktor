...
export function resolveOpencodeExecutable(bin) {
...
export function parseOpencodeModels(stdout) {
    ...
    for (const line of lines) {
        if (!MODEL_LINE_RE.test(line) || ids.has(line))
            continue;
        if (models.length >= MAX_MODELS)
            throw new Error('OpenCode model discovery exceeded the size limit');
        ids.add(line);
...
function spawnOpencode(bin, args, cwd) {
...
//# sourceMappingURL=opencode-model-catalog.js.map