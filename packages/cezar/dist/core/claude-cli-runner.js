...
import { isSignalTerminationExit, trackChildExit } from './agent-runner.js';
...
export class ClaudeCliRunner {
    ...
    constructor(opts = {}) {
        ...
        const defaultBin = process.env.CEZ_CLAUDE_BIN ??
    ...
    startSession(spec, onEvent, opts = {}) {
        ...
        const result = (async () => {
            ...
            if (eofTermTimer)
                clearTimeout(eofTermTimer);
            if (eofKillTimer)
                clearTimeout(eofKillTimer);
            if (spawnFailed)
                throw spawnFailed;
 ...
 */
export function buildClaudeArgs(spec, env = process.env) {
    const args = [
        '--input-format',
        'stream-json',
        '--output-format',
    ...
    if (spec.reasoningEffort) {
        args.push('--effort', spec.reasoningEffort);
    }
 ...
 */
export function buildAllowedTools(allowedTools, bashAllowlist) {
    const out = [];
...
function normalizeIntentionalTeardownResult(msg, terminatedByCezar) {
...
function handleClaudeMessage(msg, ctx) {
...
function waitForExit(child) {
    ...
    return new Promise((resolve) => {
        ...
        const safety = setTimeout(() => fin(child.exitCode ?? null), EOF_TERM_GRACE_MS + EOF_KILL_GRACE_MS + KILL_GRACE_MS + 5_000);
        safety.unref?.();
    });
}
function wrapSpawnError(err, bin) {
    const code = err?.code;
    if (code === 'ENOENT') {
        return new Error(`\`${bin}\` not found on PATH — install Claude Code (https://claude.com/claude-code) and run \`claude\` once to log in`);
    }
