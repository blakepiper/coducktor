    ...
    async check(definition, mode = 'execute') {
        ...
        try {
            ...
            const overlapSince = since
                ...
                : undefined;
            const result = await githubRequests.run(() => this.handle.poller.poll(this.handle.owner, this.handle.repo, definition, { since: overlapSince }));
            ...
            if (mode === 'execute')
                ...
                store.appendLog({ automationId: definition.id, revision: definition.revision, result: 'error', reason: error instanceof Error ? error.message : String(error) });
            throw error;
        }
        finally {
            if (completion)
                store.appendLog({ automationId: definition.id, revision: definition.revision, ...completion, durationMs: Date.now() - started });
            try {
                store.maybeCompact();
            }
...
function laterCursor(current, observed) {
    if (!observed)
        return current;
    if (!current)
        return observed;
