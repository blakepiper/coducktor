...
export function renderAutomationTask(definition, candidate) {
    ...
    const prompt = definition.task.prompt.replace(PLACEHOLDER_RE, (_, key) => values[key] ?? '');
...
export async function launchAutomationRun(options) {
    ...
    if (definition.task.steps) {
        ...
        if (!workflow)
            throw new Error(`unknown workflow: ${definition.task.workflow ?? 'quick-task'}`);
    }
    const input = {
    ...
    for (const run of runs)
        options.store.updateRun(run.id, { automation: provenance });
    const first = runs[0];
    if (!first)
        throw new Error('run manager did not create a run');
    return { runId: first.id };
}
/** Reserved receipts are reconciled against additive run provenance after restart. */
export function reconcileAutomationReceipts(automationStore, runStore) {
    let reconciled = 0;
    const byReceipt = new Map(runStore.listRuns().flatMap((run) => run.automation ? [[run.automation.receiptId, run.id]] : []));
