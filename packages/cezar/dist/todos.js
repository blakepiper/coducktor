...
export function todoTaskText(todo) {
...
function startWatch(dataDir) {
    const emitter = new EventEmitter();
    emitter.setMaxListeners(100);
    const entry = { emitter, watcher: undefined, timer: undefined };
    try {
...
export function onTodosChanged(dataDir, cb) {
    ...
    return () => {
        ...
        if (entry.emitter.listenerCount('changed') > 0)
            return;
        if (entry.timer)
            clearTimeout(entry.timer);
        entry.watcher?.close();
...
export function todosWatchActive(dataDir) {
