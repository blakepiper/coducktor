...
export function auxiliaryRunner(selection) {
 ...
 */
async function ownWorktreeRetention(repoRoot) {
    let raw;
    try {
        raw = await readFile(join(repoRoot, '.ai/cezar', 'config.json'), 'utf8');
    }
    ...
    try {
        ...
        if (!parsed || typeof parsed !== 'object')
            return undefined;
        const value = parsed.worktreeRetention;
        if (value === undefined)
            return undefined;
 ...
 */
export async function resolveWorktreeRetention(repoRoot) {
