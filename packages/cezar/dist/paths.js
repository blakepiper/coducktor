 ...
 */
export function cezarHomeDir(env = process.env) {
 ...
 */
export function assertCezarHomeWriteIsSandboxed(path, env = process.env) {
    if (!env.VITEST)
        return;
    const realHome = join(homedir(), '.cezar');
    if (path !== realHome && !path.startsWith(realHome + sep))
        return;
    throw new Error(`[cez] refusing to write ${path} from a test run — CEZ_HOME is not pinned to a sandbox. ` +
...
export function serverInstancesDir() {
 ...
 */
export function serverStatePath(instance = DEFAULT_SERVER_INSTANCE) {
 ...
 */
export function workspaceUiStatePath() {
...
export function providerUsagePath(env = process.env) {
 ...
 */
export function serverLockPath(instance = DEFAULT_SERVER_INSTANCE) {
    if (instance === DEFAULT_SERVER_INSTANCE)
 ...
 */
export function agentHomePaths(env = process.env) {
 ...
 */
export function claudeStateFilePath(claudeHome, env = process.env) {
