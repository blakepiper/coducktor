...
export const BIN_NAMES = ['cezar', 'cez', 'cezar-cli'];
...
export function globalShimPaths(prefix, platform = process.platform) {
    const p = platform === 'win32' ? path.win32 : path.posix;
    return BIN_NAMES.map((name) => platform === 'win32' ? p.join(prefix, `${name}.cmd`) : p.join(prefix, 'bin', name));
}
/** The directory the shims live in (for the post-install PATH hint). */
export function globalBinDir(prefix, platform = process.platform) {
    const p = platform === 'win32' ? path.win32 : path.posix;
    return platform === 'win32' ? prefix : p.join(prefix, 'bin');
}
//# sourceMappingURL=install-as-command.js.map