...
import { z } from 'zod';
import { RUN_HISTORY_PAGE_ITEMS, } from '../contract/index.js';
...
export function decodeLiveCursor(cursor) {
    const parsed = liveCursorSchema.safeParse(decodeCursor(cursor));
    if (!parsed.success)
        throw new HistoryCursorError(400, 'invalid live cursor');
    return parsed.data;
}
 ...
 */
function completeReverseLines(chunk, suffix) {
    const combined = suffix.length === 0 ? chunk : Buffer.concat([chunk, suffix]);
    const firstNewline = combined.indexOf(0x0a);
    if (firstNewline === -1)
        return { prefix: Buffer.from(combined), lines: [] };
    return {
 ...
 */
export function canonicalSessionItems(events) {
    const items = new Map();
    let turn = 0;
    let turnHasV2 = false;
    const v1Tools = [];
...
async function reverseEventsUntil(filePath, beforeSeq, wantedItems) {
    let fileSize = 0;
    try {
    ...
    if (fileSize === 0)
        return { events: [], fileSize, fileHighWater: 0, reachedStart: true, bytesRead: 0 };
    ...
    try {
        while (position > 0) {
            ...
            if (canonicalSessionItems(chronological).length >= wantedItems + 1 &&
...
async function readFileTail(filePath) {
    let fileSize = 0;
    try {
        fileSize = (await stat(filePath)).size;
    }
    ...
    if (fileSize === 0)
        return { fileSize, fileHighWater: 0, bytesRead: 0 };
    const handle = await open(filePath, 'r');
    let position = fileSize;
    let suffix = Buffer.alloc(0);
    let totalBytesRead = 0;
    try {
        while (position > 0) {
            ...
            for (let index = lines.length - 1; index >= 0; index -= 1) {
                const event = parseLine(lines[index]);
                if (event)
                    return { fileSize, fileHighWater: event.seq, bytesRead: totalBytesRead };
            }
        }
        return {
            fileSize,
...
async function forwardEventsUntil(filePath, afterSeq, wantedItems) {
    const events = [];
    ...
    try {
        ...
        for await (const line of lines) {
            ...
            if (events.length === 0 && previousBoundary)
                events.push(previousBoundary);
            events.push(event);
...
function pageEventSlice(events, selected) {
...
function forwardPageEventSlice(events, selected) {
    const lastSeq = selected.at(-1)?.lastSeq;
    if (lastSeq === undefined)
        return [];
    let end = events.length - 1;
    while (end >= 0 && events[end].seq > lastSeq)
        end -= 1;
    return events.slice(0, end + 1);
}
export async function readRunHistoryPage(filePath, cursor, onRead) {
    ...
    const reverse = forward === undefined
        ? await reverseEventsUntil(filePath, decoded?.boundarySeq ?? Number.MAX_SAFE_INTEGER, RUN_HISTORY_PAGE_ITEMS)
        : undefined;
...
const isSettledContextStatus = (status) => status !== undefined && status !== 'pending' && status !== 'running';
/** One forward pass retaining the latest Plan snapshot and only the selector-equivalent agent episode. */
export async function deriveRunContextEvents(filePath) {
    let latestPlan;
    let asOfSeq = 0;
    let turn = 0;
    const boundaries = [];
    ...
    const rootsByTurn = new Map();
    const itemIdentity = (event, id) => `${event.stepId ?? ''}:${id}`;
    const pruneSettledHistory = () => {
        ...
        for (const [key, item] of children) {
            if (item.parentId === undefined || !retainedRootIds.has(item.parentId))
                children.delete(key);
        }
        ...
        if (firstRetained === -1)
            ...
            boundaries.splice(0, firstRetained);
    };
    try {
        ...
        for await (const line of lines) {
            ...
            if (event.type === 'item.started' || event.type === 'item.updated' || event.type === 'item.completed') {
                ...
                if (typeof raw !== 'object' || raw === null || Array.isArray(raw))
                    continue;
                const item = raw;
                if (item.kind !== 'tool' || typeof item.id !== 'string' || item.id === '')
                    continue;
                ...
                if (isRoot && !existing) {
                    ...
                    if (ids)
                        ...
                        rootsByTurn.set(turn, new Set([key]));
                }
                pruneSettledHistory();
            }
        }
    ...
    pruneSettledHistory();
    const retainedRootIds = new Set([...roots.values()].map(({ id }) => id));
    const relevantChildren = [...children.values()].filter(({ parentId }) => parentId !== undefined && retainedRootIds.has(parentId));
    const contextEvents = new Map();
    if (latestPlan)
        contextEvents.set(latestPlan.seq, latestPlan);
    for (const event of boundaries)
        contextEvents.set(event.seq, event);
...
export async function readEventsAfterLiveCursor(filePath, cursor) {
    ...
    try {
        ...
        if (decoded.offset === 0)
            return { events: [], boundarySeq: decoded.boundarySeq };
        throw new HistoryCursorError(409, 'history cursor is no longer valid — reload the newest page');
    }
    ...
    return {
        events: text.split('\n').map(parseLine).filter((event) => event !== null),
        boundarySeq: decoded.boundarySeq,
    };
}
export async function validateLiveCursor(filePath, cursor) {
    const decoded = decodeLiveCursor(cursor);
    let fileSize = 0;
    try {
        fileSize = (await stat(filePath)).size;
