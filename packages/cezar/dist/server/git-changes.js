...
function unquoteGitPath(raw) {
    ...
    return inner.replace(/\\([\\"tnrf])|\\([0-7]{3})/g, (_m, esc, oct) => {
        if (oct)
...
export function patchByPath(sections) {
    const map = new Map();
    for (const section of sections) {
        const path = sectionPath(section);
        if (path != null && !map.has(path))
            map.set(path, section);
    }
    return map;
}
...
export async function collectChanges(dir, baseBranch, opts = {}) {
    if (!isSafeGitRef(baseBranch))
        return { ok: false, error: 'refusing option-like base ref' };
    ...
    try {
        ...
        const { base, repointedHead } = await resolveTaskDiffBase((args) => git(dir, args, env), baseBranch, {
...
export function assemblePayload(nameStatusOut, numstatOut, patchOut, patchCap) {
...
export async function collectRunGitStatus(dir) {
    ...
    if (remoteBranches.ok && remoteBranches.stdout.trim())
        return { branch: name, pushed: true };
    const upstream = await git(dir, ['rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{u}']);
    if (!upstream.ok)
        return { branch: name, pushed: false };
...
export function isOsOpenableImage(path) {
    const name = path.slice(path.lastIndexOf('/') + 1);
    const dot = name.lastIndexOf('.');
    if (dot <= 0)
        return false;
...
export async function createOrSwitchBranch(dir, name, from) {
    ...
    if (from) {
        ...
        if (!isSafeGitRef(from))
            return { ok: false, error: `invalid start point: ${from}` };
