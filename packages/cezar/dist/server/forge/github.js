...
function refStatusQuery(numbers) {
    const aliases = numbers
        .map((n, i) => `    r${i}: issueOrPullRequest(number: ${n}) { __typename ... on PullRequest { state isDraft reviewDecision commits(last: 1) { nodes { commit { committedDate statusCheckRollup { state } } } } reviews(last: 1, states: CHANGES_REQUESTED) { nodes { submittedAt } } reviewRequests(first: 1) { totalCount } } ... on Issue { state stateReason } }`)
        .join('\n');
    return `query ($owner: String!, $name: String!) {\n  repository(owner: $owner, name: $name) {\n${aliases}\n  }\n}`;
}
/**
...
function pushedSince(headCommittedAt, reviewedAt) {
    if (!headCommittedAt || !reviewedAt)
        return false;
...
export function readCachedRefStatuses(repoRoot, numbers) {
    ...
    for (const number of new Set(numbers)) {
        ...
        if (!hit || !hit.resolved || now - hit.at >= refStatusTtl(hit.resolved))
            continue;
...
function refStatusRecheckAfter(entry) {
...
function batchRecheckAfter(entries) {
    let soonest = null;
...
export async function fetchGithubRefStatus(repoRoot, input) {
    ...
    for (const n of wanted) {
        ...
        if (!hit || now - hit.at >= refStatusTtl(hit.resolved))
            misses.push(n);
        else
            file(n, hit.resolved);
    }
