...
export function isWsl(env = process.env, platform = process.platform, procVersion = safeProcVersion()) {
    if (platform !== 'linux')
        return false;
...
export function wslDistroName(env = process.env) {
...
export function toWindowsPath(posixPath, distro = wslDistroName()) {
...
export function translateToWindowsPath(posixPath) {
    try {
