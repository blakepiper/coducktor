...
import { z } from 'zod';
import { setWorkspaceUiStateInputSchema, taskSourceSchema, } from '../contract/index.js';
...
import { modelDiscoveryRunnerSchema, openProjectInSchema, reasoningEffortSchema, updateProjectInputSchema, ideDirectoryQuerySchema, ideFileInputSchema, ideFileQuerySchema, } from '../contract/index.js';
...
import { computeModelUsageBreakdown } from '../runs/store.js';
...
import { collectChanges, collectCommitChanges, collectRunGitStatus, collectRunCommits, commitAll, createOrSwitchBranch, imageMimeType, isOsOpenableImage, pushCurrentBranch, readWorktreePath, } from './git-changes.js';
...
import { listIdeDirectory, readIdeFile, writeIdeFile } from './ide-files.js';
import { parseRemote, resolveForge } from './forge/index.js';
import { fetchGithub, fetchGithubChecks, fetchGithubComments, fetchGithubPrDiff, fetchGithubRefStatus, forgetRefStatus, readCachedRefStatuses, refNumberFromUrl, GithubPrNotFoundError, GH_CHECKS_MAX, GH_REF_STATUS_MAX } from './github.js';
...
function foldedLength(task, stack) {
...
async function probeWritableDir(dir, create) {
...
export function createApp(deps) {
    ...
    const resolveBootProject = async (projects) => {
        if (bootProjectCache)
            return bootProjectCache;
    ...
    app.use('/api/*', async (c, next) => {
        ...
        if (MUTATING_METHODS.has(c.req.method)) {
            ...
            if (origin !== undefined) {
                ...
                const isDevProxy = c.req.header('sec-fetch-site') === 'same-origin' &&
    ...
    const refreshHealth = () => {
        ...
        healthInFlight = (async () => {
            try {
                ...
                if (changed)
                    publishHealth(payload); // only a real change reaches the wire
                return payload;
            }
            finally {
    ...
    const readHealth = async () => {
        ...
        if (!healthCache)
            return refreshHealth(); // first ever: nothing to serve yet
        const age = Date.now() - healthCache.at;
        if (age > HEALTH_MAX_STALE_MS)
            return refreshHealth(); // too old to serve — wait for the truth
        if (age > HEALTH_TTL_MS)
    ...
    const workspaceProjectsDir = async () => {
        try {
    ...
    const workspaceBrowseRoot = async () => {
        try {
    ...
    const registerFolder = async (spelled, source) => {
        ...
        if (!info?.isDirectory())
            return { status: 400, body: { error: `no such folder: ${spelled}` } };
    ...
    const workspaceConfigRoutes = new Hono()
        ...
        for (const [configuredRoot, create] of [
            ...
            if (probeError !== null)
                return c.json({ error: `not writable: ${probeError}` }, 400);
        }
        let written;
        try {
            written = await mergeWriteWorkspaceConfig((config) => {
                ...
                if (quotaRouting?.enabled !== undefined)
                    config.quotaRouting.enabled = quotaRouting.enabled;
    ...
    const noteTodoStarted = async (dataDir, todoId, taskId) => {
        try {
    ...
    const runsRoutes = new Hono()
        ...
        if (parsed.data.agentProfile !== undefined) {
            if (fallback === 'auto')
                return c.json({ error: 'an agent profile requires an explicit runner' }, 400);
        ...
        if (queued)
            return c.json({ queued: true, message: queued });
        ...
        if (target === 'default') {
            ...
            if (!relPath)
                return c.json({ error: 'path required for the default-app target' }, 400);
        ...
        if (createdNumber !== null)
            forgetRefStatus(repoRoot, createdNumber);
        store.updateRun(id, {
    ...
    const sseRoutes = new Hono()
        ...
        return streamSSENoBuffer(c, async (stream) => {
            ...
            const onDeleted = (id) => void stream.writeSSE({
                event: 'run-deleted',
    ...
    const workspaceEventsRoutes = new Hono()
        ...
        return streamSSENoBuffer(c, async (stream) => {
            ...
            const attach = (project, ctx) => {
                if (attached.has(project))
                    return;
                const { store, dataDir } = ctx;
                const onRun = (run) => void stream.writeSSE({
                    event: 'run',
    ...
    const parseRefNumbers = (raw) => {
        ...
        if (parts.length > GH_REF_STATUS_MAX)
            return null;
        const numbers = [];
    ...
    queryZodValidator(z.object({ limit: queryValue, refresh: queryValue })), async (c) => {
        ...
        .get('/github/ref-status', queryZodValidator(z.object({ prs: z.string().optional(), issues: z.string().optional() })), async (c) => {
        const { root: repoRoot } = c.get('project');
        const { prs, issues } = c.req.valid('query');
        const parsedPrs = parseRefNumbers(prs);
        const parsedIssues = parseRefNumbers(issues);
        if (parsedPrs === null || parsedIssues === null)
            return c.json({ error: 'invalid ref-status query' }, 400);
     ...
     */
    const mentionedReferenceNumbers = (run) => {
        const numbers = [];
 ...
 */
export function verifyWsUpgrade(req, bindHost) {
    const host = req.headers.host;
    const hostName = hostnameOfHost(host);
    const hosted = !resolveCapabilities(process.env, bindHost).localHandoff;
    if (!hosted && !isLoopbackHostHeader(hostName))
        return false;
    const origin = req.headers.origin;
    if (origin === undefined)
        return { trusted: true }; // non-browser client — no Origin to spoof
    ...
    const originHost = hostnameOfOrigin(origin);
    if (originAuthority === null || !originHost)
        return false;
    if (originAuthority === authorityOfHost(host))
        return { trusted: true }; // the cockpit itself
    ...
    if (fetchSite !== undefined && fetchSite !== 'same-origin')
        return false;
    if (!isLoopbackHostHeader(originHost) || !isLoopbackHostHeader(hostName))
        return false;
 ...
 *  `authorityOfHost`. */
function authorityOfOrigin(origin) {
    try {
 ...
 */
export function resumeCommand(runner, sessionId) {
