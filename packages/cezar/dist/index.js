...
import { createQuotaRuntime } from './core/quota/runtime.js';
import { formatUsageReport, readUsageReport } from './core/quota/usage-report.js';
...
// ---- run (headless) ----------------------------------------------------------
async function usageCommand(repoRoot, json, refresh) {
    const quotaRuntime = await createQuotaRuntime(repoRoot);
    try {
        ...
        if (json)
            ...
            console.log(formatUsageReport(providers));
    }
    finally {
        quotaRuntime.dispose();
    }
