...
function git(cwd, args) {
    return new Promise((resolve) => {
        execFile('git', args, { cwd, encoding: 'utf8' }, (err, stdout) => resolve({ ok: !err, stdout: stdout ?? '' }));
    });
}
...
async function ensureExcluded(commonGitDir, line) {
 ...
 */
export async function seedAgentConfigLocalLayer(repoRoot, worktreeCwd, env = process.env) {
    if (worktreeCwd === repoRoot)
        return [];
    ...
    for (const def of CONFIG_FILES) {
        ...
        if (rel.startsWith('..') || !(await fileExists(src)))
            continue;
